...
class EnglishFrenchDataset(Dataset):
    def __init__(self, encoder_inputs, decoder_inputs, decoder_outputs):
        """
        Dataset for English to French translation
        
        Args:
            encoder_inputs: Padded English sequences
            decoder_inputs: Padded French sequences for input to decoder
            decoder_outputs: Padded French sequences for output from decoder
        """
...
class TranslationDataLoader:
    def __init__(self, max_english_length=None, max_french_length=None, batch_size=32):
        """
        ...
        self.english_idx = 3  # Starting index for new words
        self.french_idx = 3
    ...
    def build_vocabulary(self, sentences, is_french=False):
        """
        Build vocabulary from sentences
        
        Args:
            sentences: List of sentences
            is_french: Whether this is French vocabulary
            
        Returns:
            Updated word2idx and idx2word dictionaries
        """
        word2idx = self.french_word2idx if is_french else self.english_word2idx
        idx2word = self.french_idx2word if is_french else self.english_idx2word
        next_idx = self.french_idx if is_french else self.english_idx
    ...
    def prepare_data(self, file_path=None, content=None):
        ...
        # Build vocabularies
        self.english_word2idx, self.english_idx2word = self.build_vocabulary(english_sentences)
        self.french_word2idx, self.french_idx2word = self.build_vocabulary(french_sentences, is_french=True)
        ...
        # Prepare encoder inputs (English)
        encoder_inputs = []
        ...
        self.full_dataset = EnglishFrenchDataset(
            encoder_inputs,
            decoder_inputs,
            decoder_outputs
        )
...
# Example usage:
def main():
    # Initialize data loader (removed test_size parameter)
