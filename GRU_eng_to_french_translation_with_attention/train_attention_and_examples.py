...
def train_translation_model(data_path, output_dir, batch_size=32, embedding_size=256, 
    ...
    max_french_length = data['max_french_length']
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch % save_interval == 0 or epoch == num_epochs:
            ...
            metrics_df.to_csv(f"{output_dir}/training_metrics_attention.csv", index=False)
...
def translate_examples(model, data_loader, english_sentences, english_word2idx, french_idx2word, 
    ...
    Args:
        ...
        english_word2idx: English word to index mapping
        french_idx2word: French index to word mapping
    ...
    indices = np.random.choice(len(english_sentences), min(num_examples, len(english_sentences)), replace=False)
    
    for idx in indices:
        english_sentence = english_sentences[idx]
        print(f"\nEnglish: {english_sentence}")
        
        # Convert sentence to indices
        tokens = english_sentence.split()
        input_indices = [english_word2idx.get(word, english_word2idx['< SOS >']) for word in tokens]
        
        # Add EOS token
        input_indices.append(english_word2idx['<EOS>'])
        
        # Pad sequence
        src_len = len(input_indices)
        padding = data_loader.max_english_length - src_len
        input_indices.extend([english_word2idx['<PAD>']] * padding)
        ...
        with torch.no_grad():
            translations, attention_weights = model.translate(
                src=src_tensor,
                max_len=max_len,
                sos_idx=data_loader.french_word2idx['< SOS >'],
                eos_idx=data_loader.french_word2idx['<EOS>']
            )
        
        # Convert back to words
        for translation in translations:
            french_translation = indices_to_sentence(translation.cpu().numpy(), french_idx2word)
            print(f"French (translated): {french_translation}")
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = "translation_model"  # Directory to save outputs
    ...
    translate_examples(
        ...
        english_sentences=data['all_english_sentences'],
        english_word2idx=data['english_word2idx'],
        french_idx2word=data['french_idx2word'],
        device=DEVICE,
        num_examples=5
