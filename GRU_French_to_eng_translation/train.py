...
MODEL_SAVE_DIR = "/data1/Sadia/Hamed_GRU/models2"
LOG_DIR = "/data1/Sadia/Hamed_GRU/logs2"

# Ensure directories exist
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)
os.makedirs(LOG_DIR, exist_ok=True)

def calculate_word_accuracy(predictions, targets, english_idx2word, english_word2idx):
    """
    ...
    Args:
        ...
        english_idx2word: Index to word mapping for output language
        english_word2idx: Word to index mapping for output language
        
    Returns:
        accuracy: Word-level accuracy
    """
    pad_idx = english_word2idx['<PAD>']
    eos_idx = english_word2idx['<EOS>']
    ...
    for i in range(predictions.size(0)):
        ...
        for idx in pred_seq:
            ...
            if idx_item != pad_idx and idx_item != eos_idx:
                if idx_item in english_idx2word:
                    pred_words.append(english_idx2word[idx_item])
        ...
        for idx in target_seq:
            ...
            if idx_item != pad_idx and idx_item != eos_idx:
                if idx_item in english_idx2word:
                    target_words.append(english_idx2word[idx_item])
...
def evaluate(model, val_loader, criterion, device, english_idx2word, english_word2idx):
    """
    Evaluate the model
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(val_loader):
            ...
            # Calculate accuracy
            batch_accuracy = calculate_word_accuracy(pred_tokens, trg_tokens, english_idx2word, english_word2idx)
...
def main():
    print("Starting French-to-English translation training...")
    ...
    criterion = nn.CrossEntropyLoss(ignore_index=translation_loader.english_word2idx['<PAD>'])
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    
    # Create CSV file for logging
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = os.path.join(LOG_DIR, f"training_log2_{timestamp}.csv")
